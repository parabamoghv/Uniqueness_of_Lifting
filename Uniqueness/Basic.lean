...
lemma is_open_of_is_open_coe (Y:Type _) [TopologicalSpace Y] (A: Set Y)
    ...
    constructor
    rintro v ⟨ vW,vV⟩
    apply (@Set.inter_subset_left Y A U)
    rw [← hW1]
    constructor
    exact vW
    apply VU
    ...
    constructor
    apply @Set.inter_subset_left Y W U
    rw [hW1]
    constructor
    exact xA
    apply VU
...
if the triangles commute then H₁=H₂.
...
/- Define S := {y ∈ Y ∣ H₁(y) = H₂(y)} -/
  ...
  have ClopenS : IsClopen S := by
    ...
    use  U_y
    use UyNy
    have inv_prop := @Set.preimage_setOf_eq Y U_y (fun y=> H₁ y = H₂ y) (@Subtype.val Y fun x ↦ x ∈ U_y : { x // x ∈ U_y } → Y)
    ...
    exact TrivN.continuousOn_toFun.comp_continuous (H₁Cont.comp continuous_subtype_val) H₁USource
